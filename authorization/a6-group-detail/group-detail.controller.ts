...
import { GetLanguage } from '@decorator/get-language.decorator';
...
export default class GroupDetailController {
  ...
  async findAll(
    ...
    if (result.length > 0) {
      // Update name by lang
      const localizedResult = result.map((item: any) => ({
        ...item,
        name: item.name[lang],
      }));
  ...
  async create(@Body() body: CreateGroupDetailDto): Promise<any> {
    ...
    let result = await this.groupDetailService.findOneBy(filter);

    // if has result => result
    if (!result) {
      // add to admin
      result = await this.groupDetailService.create(body);
    }

    const groupDetailItem: GroupDetailType = {
      idGroupDetail: result._id,
      accessMethods: [
        MethodRouteEnum.GET,
  ...
  async getAllGroupDetailsOfUser(
    ...
    if (groupDetails.length > 0) {
      // Update name by lang
      const localizedResult = groupDetails.map((item: any) => ({
        ...item,
        name: item.name[lang],
      }));
