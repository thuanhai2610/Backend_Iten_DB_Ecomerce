...
import { AggregateOptions, FilterQuery, PipelineStage, Types } from 'mongoose';
...
export default class BaseService<T> {
  ...
  async createMany(items: any[]) {
    return this.baseRepository.createMany(items);
  }

  async distinct(field: string, filter?: FilterQuery<T>) {
    return this.baseRepository.distinct(field, filter);
  }
  ...
  async aggregate(pipeline?: PipelineStage[], options?: AggregateOptions) {
    return this.baseRepository.aggregate(pipeline, options);
  }
